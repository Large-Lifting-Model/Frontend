...
function Workout() {
	//const navigate = useNavigation(); // navigate makes rendering not work.  Not sure why.
	...
	const handleSubmit = (event) => {
		...
		alert(`Submitting refinement:  "${refinement}". This is a placeholder for API call`);
		...
		<main className={styles.workout}>
			...
			<section>
				...
				<div style={{width: '100%'}}>
					...
					<button type="button" className={buttonStyles.primary} style={{width: '100%'}} onClick={() => handleApprove()}>Confirm Workout</button>
				</div>
				:
				<div className={styles.container}>
					<button type="button" className={buttonStyles.back} onClick={() => handleDidNotFinish()}>Did not Finish</button>
					<button type="button" className={buttonStyles.primary} onClick={() => handleFinished()}>Finished Workout</button>
				</div>
				}			
			</section>
		</main>
